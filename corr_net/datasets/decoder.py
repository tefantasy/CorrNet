...
def temporal_sampling_indices(frames_length, start_idx, end_idx, num_samples):
    """
    Same as `temporal_sampling`, but instead returns frame indices for 
    loading extracted frames. 
    Args:
        frames_length (int): total number of frames in the frame sequence.
    """
    index = torch.linspace(start_idx, end_idx, num_samples)
    index = torch.clamp(index, 0, frames_length - 1).long()
    return index
...
def decode(
    ...
    # Perform temporal sampling from the decoded video. **with uniform intervals**
...
def decode_seq(
    ...
    """
    Decode tar wrapped frame sequence and perform temporal sampling.
    Args:
        tar_handler: tarfile handler.
        frames_list (list): file names of all frames in the tar file. 
        (other args): same as in `decode` function. 
    Returns:
        frames (tensor): decoded frames from the tar file.
    """
    assert clip_idx >= -1, "Not valied clip_idx {}".format(clip_idx)
    assert video_meta is not None

    fps = video_meta['fps']
    frames_length = video_meta['num_frames']
    duration = video_meta['duration']

    # select frames to be decoded
    clip_size = sampling_rate * num_frames / target_fps * fps
    start_idx, end_idx = get_start_end_idx(
        frames_length,
        clip_size,
    ...
    frame_indices = temporal_sampling_indices(
                        frames_length, start_idx, end_idx, num_frames)

    frames = []
    try:
        for idx in frame_indices:
            # file name of frames start from 1
            filename = self.cfg.DATA.FRAME_SEQUENCE_TEMPLATE.format(int(idx) + 1)
            img = Image.open(io.BytesIO(tar.extractfile(filename).read())).convert('RGB')
            frames.append(np.asarray(img))
    except Exception as e:
        print("Failed to decode image with exception: {}".format(e))
        return None
